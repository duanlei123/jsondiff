...
var jsonCompositeTemplate = '<div id="json-composite">' +
    ...
    '<h1>JSON Lint Pro</h1>' +
    '<h2>验证和格式化JSON的最简单方法</h2>' +
    '<ul><li>输入JSON，或者输入一个远程URL</li>' +
    '<li>使用分割模式来加速您的工作流程，或运行差异。</li>' +
    '<li>使用delete按钮快速清除输入。</li>' +
    '</ul>' +
    '</div>' +
    '</div>',

    validatorTemplate = '<form class="JSONValidate" method="post" action="" name="JSONValidate">' +
        '<textarea id="json_input1" class="json_input" name="json_input" class="json_input" rows="30" cols="100" spellcheck="false" placeholder="输入json来验证…"></textarea>' +
        '<a href="#" title="格式化json数据" class="button validate">' +
        '<span class="icon">Lint Me!</span>' +
        '</a>' +
        '<a href="#" title="隐藏当前页" class="button split-view">' +
        '<span class="icon">Split View</span>' +
        '</a>	' +
        '<a href="#" title="删除当前数据" class="button reset">' +
        '<span class="icon">Reset</span>' +
        '</a>' +
        '<a href="#" title="运行验证并执行差异" class="button diff">' +
        '<span class="icon">Diff</span>' +
        '</a>' +
        '</form>',
    errorTemplate = '<div class="error-view">' +
        '<a class="close-btn" href="#">X</a>' +
        '<span class="arrow-down"></span>' +
        '<pre class="results"></pre>' +
        '</div>',

    diffTemplate = '<div id="diff-view">' +
        '<a href="#" title="运行验证并执行差异" class="button diff">' +
        '<span class="icon">Diff</span>' +
        '</a>' +
        '<a href="#" title="取消 diff" class="button cancel-diff">' +
        '<span class="icon">Cancel diff</span>' +
        '</a>' +
        '<div class="json_input" contenteditable="true">' +
        '</div>' +
        '</div>';
...
$(function () {
    ...
    //获取请求1按钮
    $("#reqiest1").click(function(){
         //获取请求url, 和 请求方法
        var url = $.trim($("#requestinput1").val())
        var method = $.trim($("#requesmethod1").text())
        var heards = $.trim($("#requestheards1").val())
        var bodys =  $.trim($("#requestbody1").val())
        if(isNull(url,method)){
            //发送请求
            $.ajax({
                url: '/getRequest',
                data: {"url":url,"method":method,"heards":heards,"bodys":bodys},
                type: 'GET',
                dataType: "json",
                success: function(data) {
                   //赋值
                   $(".JSONValidate.primary textarea").val(JSON.stringify(data, null, 4));
                }
            });
        }else{
            alert("正确选择method或正确填写URL")
        }
    });

    //获取请求2按钮
     $("#reqiest2").click(function(){
         //获取请求url, 和 请求方法
         var url = $.trim($("#requestinput2").val())
         var method = $.trim($("#requesmethod2").text())
         var heards = $.trim($("#requestheards2").val())
         var bodys =  $.trim($("#requestbody2").val())
        if(isNull(url,method)){
        //发送请求
            $.ajax({
                url: '/getRequest',
                ...
                success: function(data) {
                   //赋值
                   $(".JSONValidate.primary2 textarea").val(JSON.stringify(data, null, 4));
                }
            });
        }else{
            alert("正确选择method或正确填写URL")
    ...
    $("#clearText").click(function(){
        $(".JSONValidate.primary2 textarea").val('');
        $(".JSONValidate.primary textarea").val('');
        $("#requestinput1").val('');
        $("#requestinput2").val('');
    })
});
